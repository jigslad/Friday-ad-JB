...
EOF
    ...
    protected function sendBusinessUserForModeration($searchParam, $input, $output, $isForManualModeration, $manualModerationReason)
    {
        ...
        for ($i = 0; $i <= $count;) {
            ...
            $command = $this->getContainer()->getParameter('fa.php.path').$memoryLimit.' '.$this->getContainer()->get('kernel')->getRootDir().'/console fa:send:business-user-for-moderation '.$commandOptions.' -v';
