...
EOF
    ...
    protected function updateSolrIndex($solrClient, $searchParam, $input, $output)
    {
        ...
        for ($i = 0; $i <= $count;) {
            ...
            $command = $this->getContainer()->getParameter('fa.php.path').$memoryLimit.' '.$this->getContainer()->getParameter('project_path').'/console fa:update:user-shop-detail-solr-index'.$commandOptions.' '.$input->getArgument('action');
