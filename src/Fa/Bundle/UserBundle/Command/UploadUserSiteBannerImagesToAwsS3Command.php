 ...
 *
 * php bin/console fa:upload-user-site-banner:image-s3  --user_id="xxxx"
 *
 * @author Rohini <rohini.subburam@fridaymediagroup.com>
 * @copyright 2019 Friday Media Group Ltd
 * @version v1.0
 */
class UploadUserSiteBannerImagesToAwsS3Command extends ContainerAwareCommand
{
    /**
     * Configure.
     */
    ...
    {
        $this
        ->setName('fa:upload-user-site-banner:image-s3')
        ->setDescription('Upload user site banner to s3')
    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
        ...
        while (!$done) {
            ...
            if ($ids) {
                foreach ($ids as $id) {
                    $imagePath  = $userImageDir.CommonManager::getGroupDirNameById($id);
                    $userSiteBannerManager = new UserSiteBannerManager($this->getContainer(), $id, null, $imagePath);
                    $userSiteBannerManager->uploadImagesToS3($id);
