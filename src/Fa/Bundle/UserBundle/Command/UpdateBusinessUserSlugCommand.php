...
EOF
    ...
    protected function updateBusinessUserSlug($searchParam, $input, $output)
    {
        ...
        for ($i = 0; $i <= $count;) {
            ...
            $command = $this->getContainer()->getParameter('fa.php.path').$memoryLimit.' '.$this->getContainer()->get('kernel')->getRootDir().'/console fa:update:business:user-slug'.$commandOptions.' '.$input->getArgument('action');
