...
EOF
    ...
    protected function updateUserTotalAdCount($input, $output)
    {
        ...
        for ($i = 0; $i <= $count;) {
            ...
            $command = $this->getContainer()->getParameter('fa.php.path').$memoryLimit.' '.$this->getContainer()->getParameter('project_path').'/console fa:update:user-ad-statistics '.$commandOptions.' '.$input->getArgument('action');
