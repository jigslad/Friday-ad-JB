...
EOF
    ...
    protected function updateSolrIndexWithOffset($solrClient, $searchParam, $input, $output)
    {
        ...
        foreach ($userPackages as $userPackage) {
            ...
            $command = $this->getContainer()->getParameter('fa.php.path').$memoryLimit.' '.$this->getContainer()->getParameter('project_path').'/console fa:update:user-ad-shop-detail'.$commandOptions;
    ...
    protected function updateSolrIndex($solrClient, $searchParam, $input, $output)
    {
        ...
        for ($i = 0; $i < $count;) {
            ...
            $command = $this->getContainer()->getParameter('fa.php.path').$memoryLimit.' '.$this->getContainer()->getParameter('project_path').'/console fa:update:all-user-shop-detail-solr-index'.$commandOptions;
            $output->writeln($command, true);
            passthru($command, $returnVar);
