    ...
    public function save($event)
    {
        ...
        if (is_object($user)) {
            ...
            if ($dotmailer->getOptIn() != 1) {
                ...
                exec('nohup'.' '.$this->container->getParameter('fa.php.path').' bin/console fa:dotmailer:unsubscribe-contact --id='.$dotmailer->getId().' >/dev/null &');
                // remove contact from dotmailer.
                exec('nohup'.' '.$this->container->getParameter('fa.php.path').' bin/console fa:dotmailer:delete-contact --email='.$dotmailer->getEmail().' >/dev/null &');
            }
