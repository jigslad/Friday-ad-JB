    ...
    public function updateAction(Request $request, $id)
    {
        ...
        if ($formManager->isValid($form)) {
            ...
            if ($form->get('is_private_phone_number')->getData() && $oldPhoneNumber != $form->get('phone')->getData()) {
                exec('nohup'.' '.$this->container->getParameter('fa.php.path').' bin/console fa:update:user-ad-yac-number edit --user_id='.$id.' >/dev/null &');
            }
            ...
            if ($oldIsPrivatePhoneNumber != $form->get('is_private_phone_number')->getData()) {
                if ($form->get('is_private_phone_number')->getData()) {
                    exec('nohup'.' '.$this->container->getParameter('fa.php.path').' bin/console fa:update:user-ad-yac-number allocate --user_id='.$id.' >/dev/null &');
                } elseif (!$form->get('is_private_phone_number')->getData()) {
                    exec('nohup'.' '.$this->container->getParameter('fa.php.path').' bin/console fa:update:user-ad-yac-number setsold --user_id='.$id.' >/dev/null &');
                }
            }
        ...
        if ($formManager->isValid($form)) {
            try {
                foreach ($entities as $entity) {
                    ...
                    /*if ($entity->getStatus() && $entity->getStatus()->getId() == EntityRepository::USER_STATUS_ACTIVE_ID) {
                        $command = $this->container->getParameter('fa.php.path').' bin/console fa:update:ad-solr-index update --user_id="'.$entity->getId().'" --status="A,S,E"';
                        passthru($command, $returnVar);
                        //exec('nohup'.' '.$this->container->getParameter('fa.php.path').' bin/console fa:update:ad-solr-index add --user_id="'.$entity->getId().'" --status="A,S,E" >/dev/null &');
                    }*/
                    $this->getEntityManager()->getConnection()->commit();
                }
