...
class UserSiteBannerController extends CoreController
{
    ...
    public function ajaxSaveUploadedImageAction(Request $request)
    {
        if ($request->ismethod('post')) {
            ...
            try {
                if ($form->isValid()) {
                    exec('nohup'.' '.$this->container->getParameter('fa.php.path').' bin/console fa:send:business-user-for-moderation --userId='.$user->getId().' >/dev/null &');
                } else {
        ...
        if ($this->isAuth() && $request->isXmlHttpRequest()) {
            ...
            if ($userSiteObj && $userSiteBanner) {
                ...
                if ($bannerUpdated) {
                    ...
                    exec('nohup'.' '.$this->container->getParameter('fa.php.path').' bin/console fa:send:business-user-for-moderation --userId='.$userSiteObj->getUser()->getId().' >/dev/null &');
                } else {
