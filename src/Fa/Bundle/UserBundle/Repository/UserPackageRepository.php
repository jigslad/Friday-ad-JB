    ...
    public function assignPackageToUser(User $user, Package $package, $remark = null, $paymentId = null, $expired = false, $container = null, $ad_update = true)
    {
        ...
        if ($user && $user->getBusinessCategoryId() && $container && $ad_update) {
            if (in_array($user->getBusinessCategoryId(), array(CategoryRepository::ADULT_ID, CategoryRepository::SERVICES_ID))) {
                exec('nohup'.' '.$container->getParameter('fa.php.path').' '.$container->get('kernel')->getRootDir().'/console fa:update:user-shop-detail-solr-index --id='.$user->getId().' delete >/dev/null &');
                exec('nohup'.' '.$container->getParameter('fa.php.path').' '.$container->get('kernel')->getRootDir().'/console fa:update:user-shop-detail-solr-index --id='.$user->getId().' >/dev/null &');
            } elseif (!in_array($user->getBusinessCategoryId(), array(CategoryRepository::ADULT_ID, CategoryRepository::SERVICES_ID))) {
                exec('nohup'.' '.$container->getParameter('fa.php.path').' '.$container->get('kernel')->getRootDir().'/console fa:update:user-ad-shop-detail --user_id='.$user->getId().' >/dev/null &');
            }
