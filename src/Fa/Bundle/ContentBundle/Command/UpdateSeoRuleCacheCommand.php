...
EOF
    ...
    protected function updateSeoRuleCache($searchParam, $action, $page, $input, $output)
    {
        ...
        for ($i = 0; $i <= $count;) {
            ...
            $command = $this->getContainer()->getParameter('fa.php.path').$memoryLimit.' bin/console fa:update:seo:rule:cache '.' '.$input->getArgument('action').' '.$input->getArgument('page').' '.$commandOptions;
