...
EOF
    ...
    protected function updateSeoRuleCache($searchParam, $action, $page, $input, $output)
    {
        ...
        for ($i = 0; $i <= $count;) {
            ...
            $command = $this->getContainer()->getParameter('fa.php.path').$memoryLimit.' '.$this->getContainer()->getParameter('project_path').'/console fa:update:seo:rule:cache '.' '.$input->getArgument('action').' '.$input->getArgument('page').' '.$commandOptions;
