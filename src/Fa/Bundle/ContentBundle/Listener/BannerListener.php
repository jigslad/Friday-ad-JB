    ...
    private function removeBannerCache($objBanner)
    {
        if ($objBanner->getBannerPages()) {
            foreach ($objBanner->getBannerPages()->toArray() as $bannerPage) {
                ...
                //exec('nohup'.' '.$this->container->getParameter('fa.php.path').' '.$this->container->get('kernel')->getRootDir().'/console fa:update:banner:cache generate '.$bannerPage->getId().' >/dev/null &');
            }
        }
    }
}
