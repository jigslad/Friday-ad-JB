 ...
 * @version 1.0
 */
class RemoveWightbayFeedAdCommand extends ContainerAwareCommand
{
    /**
     * Configure.
     */
    ...
    {
        $this
        ->setName('fa:remove:wightbay-feed-ad')
        ->setDescription("Remove wightbay ads")
...
Actions:
Command:
   php app/console fa:remove:wightbay-feed-ad
EOF
        );
    }
    ...
    protected function updateDimension($searchParam, $input, $output)
    {
        ...
        for ($i = 0; $i <= $count;) {
            ...
            $command = $this->getContainer()->getParameter('fa.php.path').$memoryLimit.' '.$this->getContainer()->getParameter('project_path').'/console fa:remove:wightbay-feed-ad -v '.$commandOptions.' '.$input->getArgument('action');
