...
use Fa\Bundle\AdFeedBundle\lib\CandP;
 ...
 * @version 1.0
 */
class ExportCandPFeedCommand extends ContainerAwareCommand
{
    /**
     * Configure.
     */
    ...
    {
        $this
        ->setName('fa:export-candp-feed')
        ->setDescription("Export CandP-feed")
...
Actions:
- Export CandP feed data

Command:
 - php app/console fa:export-candp-feed'
EOF
        );
    }
    ...
    protected function searchResultWithOffset($input, $output)
    {
        ...
        if ($category == 'Vehicles') {
            ...
            $objCandP = new CandP($this->getContainer());
            $objCandP->writeVehiclesData($ads);
        } else {
    ...
    protected function getAds($step, $offset, $category, $count = false)
    {
        ...
        if ($category == 'Vehicles') {
            ...
            $data['query_filters']['item']['is_trade_ad'] = 0;
    ...
    protected function searchResult($input, $output)
    {
        ...
        for ($i = 0; $i <= $count;) {
            ...
            $command = $this->getContainer()->getParameter('fa.php.path').$memoryLimit.' '.$this->getContainer()->getParameter('project_path').'/console fa:export-candp-feed -v '.$commandOptions;
