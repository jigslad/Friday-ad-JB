 ...
 * @version 1.0
 */
class InsertFeedAdCommand extends ContainerAwareCommand
{
    /**
     * Configure.
     */
...
Actions:
Command:
   php app/console fa:feed:update
EOF
        );
    }
        ...
        foreach ($FeedAds as $FeedAd) {
            if($FeedAd->getStatus()!='R') {
                try {
                    ...
                    $text = '';
                    if ($FeedAd->getAdText()) {
                        $text = $FeedAd->getAdText();
                    }
                    $output->writeln('Exception for: '.$text, true);
                    $output->writeln('Exception: '.$e->getTraceAsString(), true);
                }
            }
        }
    ...
    protected function updateDimension($searchParam, $input, $output)
    {
        ...
        for ($i = 0; $i <= $count;) {
            ...
            $command = $this->getContainer()->getParameter('fa.php.path').$memoryLimit.' '.$this->getContainer()->get('kernel')->getRootDir().'/console fa:feed:update -v '.$commandOptions.' '.$input->getArgument('action');
            $output->writeln($command, true);
            passthru($command, $returnVar);
