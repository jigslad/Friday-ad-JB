...
EOF
    ...
    protected function searchResult($input, $output)
    {
        ...
        for ($i = $step; $i <= ($count+$step);) {
            ...
            $command = $this->getContainer()->getParameter('fa.php.path').$memoryLimit.' '.$this->getContainer()->getParameter('project_path').'/console fa:export-news-now-feed -v '.$commandOptions;
