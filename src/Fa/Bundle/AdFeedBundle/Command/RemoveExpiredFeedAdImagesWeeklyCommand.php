...
/**
 * This command is used to remove expired feed ads weekly.
 *
 * @author Amit Limbadia <amitl@aspl.in>
 * @copyright 2014 Friday Media Group Ltd
 * @version v1.0
 */
class RemoveExpiredFeedAdImagesWeeklyCommand extends ContainerAwareCommand
{
    /**
    ...
    {
        $this
        ->setName('fa:remove:expired-feed-ad-images-weekly')
...
Command:
 - php app/console fa:remove:expired-feed-ad-images-weekly
EOF
        );
    }
    ...
    protected function removeExpiredFeedAdImages($input, $output)
    {
        ...
        for ($i = 0; $i < $count;) {
            ...
            $command = $this->getContainer()->getParameter('fa.php.path').$memoryLimit.' '.$this->getContainer()->getParameter('project_path').'/console fa:remove:expired-feed-ad-images-weekly '.$commandOptions;
    ...
    {
        list($startDate, $endDate) = $this->getWeeklyDateInTimeStamp();
        ...
        $query = $adFeedRepository->getBaseQueryBuilder()
            ...
            ->andWhere('('.AdFeedRepository::ALIAS.'.created_at BETWEEN '.$startDate.' AND  '.$endDate.') OR ('.AdFeedRepository::ALIAS.'.updated_at BETWEEN '.$startDate.' AND  '.$endDate.')');
     ...
     * @return array
     */
    private function getWeeklyDateInTimeStamp()
    {
        $startDate = CommonManager::getTimeStampFromStartDate(date('Y-m-d', (strtotime(date('Y-m-d'))- 24*60*60*7)));
