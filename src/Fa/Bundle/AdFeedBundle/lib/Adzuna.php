...
class Adzuna extends Export
{
    const VEHICLE_FILE = 'AdzunaMotorsXML.xml';
    ...
    public function fixFile($category)
    {
        ...
        $command = "sed -i '1s/^/<adzuna>/' ".$this->rootDir().'/export/xml/'.$file;
        ...
        $command = "sed -i '\$a</adzuna>' ".$this->rootDir().'/export/xml/'.$file;
        passthru($command, $returnVar);
