...
class EntityRepository extends BaseEntityRepository
{
     ...
     * @return array|mixed
     */
    public function getAllEntities($container)
    {
        if ($container) {
            $culture     = CommonManager::getCurrentCulture($container);
            $tableName   = $this->getEntityTableName();
        ...
        $entityArray  = array();
        $queryBuilder = $this->getBaseQueryBuilder()
            ->innerJoin(self::ALIAS.'.category_dimension', CategoryDimensionRepository::ALIAS);
        $queryBuilder->where(self::ALIAS.'.status = 1');
        $entities = $queryBuilder->getQuery()->getResult();

        foreach ($entities as $entity) {
            $entityArray[$entity->getId()]['slug'] = $entity->getSlug();
            $entityArray[$entity->getId()]['name'] = $entity->getName();
            $entityArray[$entity->getId()]['category_dimension_id'] = $entity->getCategoryDimension()->getId();
            $entityArray[$entity->getId()]['category_dimension_name'] = $entity->getCategoryDimension()->getName();
        }
     ...
     * @param $container
     * @param $id
     * @param null $adRepository
     * @return array
     */
    public function getCachedEntityById($container, $id, $adRepository = null)
    {
        if (! empty($id)) {
            $entities = CommonManager::getCacheVersion($container, 'entity|getAllEntities|en_GB');

            $entity = [
                'id' => $id,
                'name' => $entities[$id]['name'],
                'slug' => $entities[$id]['slug'],
                'category_dimension_name' => $entities[$id]['category_dimension_name']
            ];
            if ($adRepository) {
                $entity['listing_class'] = $adRepository->getListingClass($entities[$id]['category_dimension_name']);
            }
