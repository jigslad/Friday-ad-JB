 ...
 * @version 1.0
 */
class AdReportExportToCsvCommand extends ContainerAwareCommand
{
    /**
    ...
    {
        $this
        ->setName('fa:ad:report:export-to-csv')
...
EOF
    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
        ...
        if (isset($offset)) {
            ...
            $adReportDailyRepository = CommonManager::getHistoryRepository($container, 'FaReportBundle:AdReportDaily');
            ...
            if (isset($searchParam['search']['csv_name']) && $searchParam['search']['csv_name']) {
                ...
                $fileName = "AdReport_".date('d-m-Y H:i:s').'.tmp';
            }
    ...
    protected function adReportExportToCsvWithOffset($searchParam, $input, $output)
    {
        ...
        if (count($adReports) > 0) {
            ...
            foreach ($adReports as $adReport) {
                ...
                foreach ($searchParam['search']['report_columns'] as $reportColumn) {
                    if ($reportColumn == 'print_edition_ids' && isset($adPrintDates[$adReport['ad_id']]) && isset($adPrintDates[$adReport['ad_id']][$adReport['id']]) && isset($adPrintEditions[$adReport['ad_id']]) && isset($adPrintEditions[$adReport['ad_id']][$adReport['id']])) {
                        $printEditions = array();
                        foreach ($adPrintDates[$adReport['ad_id']][$adReport['id']] as $adPrintInsertDate) {
                            $printEditions = array_merge($printEditions, (isset($adPrintEditions[$adReport['ad_id']][$adReport['id']][$adPrintInsertDate]) ? $adPrintEditions[$adReport['ad_id']][$adReport['id']][$adPrintInsertDate] : array()));
                        }
                        $printEditions = array_unique($printEditions);
                        $printEditionString = '';
    ...
    protected function adReportExportToCsv($searchParam, $input, $output)
    {
        ...
        for ($i = 0; $i <= $count;) {
            ...
            $command = $this->getContainer()->getParameter('fa.php.path').$memoryLimit.' '.$this->getContainer()->getParameter('project_path').'/console fa:ad:report:export-to-csv '.$commandOptions;
        ...
        if (isset($searchParam['search']['csv_email']) && $searchParam['search']['csv_email']) {
            ...
            ->setBody(
                $this->getContainer()->get('templating')->render(
                    'FaReportBundle:AdReportAdmin:email.html.twig',
    ...
    protected function getAdReportCount($searchParam)
    {
        if (in_array('total_ads', $searchParam['search']['report_columns'])) {
            ...
            $qb = CommonManager::getHistoryRepository($this->getContainer(), 'FaReportBundle:AdReportDaily')->getAdReportQuery($searchParam['search'], $searchParam['sort'], $this->getContainer(), true);
