 ...
 * @version 1.0
 */
class ExportUserReportCommand extends ContainerAwareCommand
{
    /**
...
Command:
 - php app/console fa:export-user-report --criteria="xxxx"
EOF
        );
    }
    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
        ...
        if (isset($criteria['rus_csv_name']) && $criteria['rus_csv_name'] && !isset($offset)) {
            ...
            $fileName = "UserReport_".date('d-m-Y H:i:s').".tmp";
        }
        ...
        if ($criteria['rus_report_type'] == 'user_wise') {
            ...
            if (CommonManager::inArrayMulti($reportDailyFields, $criteria['rus_report_columns'])) {
                $resultArray = CommonManager::getHistoryRepository($container, 'FaReportBundle:UserReportDaily')->getUserReportDailyTotalSum($criteria);
            } else {
                $resultArray[0] = array();
            }
            ...
            $categoryAndEditionDataArray = CommonManager::getHistoryRepository($container, 'FaReportBundle:UserReport')->getCategoryAndEditionDataArray($criteria, $container);

            if (CommonManager::inArrayMulti($reportBooleanFields, $criteria['rus_report_columns'])) {
                $booleanAndOtherFieldsArray = CommonManager::getHistoryRepository($container, 'FaReportBundle:UserReport')->getBooleanAndOtherFieldsSumQuery($criteria)->getResult();
            }

            if (CommonManager::inArrayMulti(array('is_new', 'signup_date'), $criteria['rus_report_columns'])) {
                $newlySignupArray = CommonManager::getHistoryRepository($container, 'FaReportBundle:UserReport')->getNewlySignupUsersQuery($criteria)->getResult();
            }
            ...
            if (isset($criteria['rus_csv_name']) && $criteria['rus_csv_name'] && !isset($offset)) {
                ...
                $fileName = "UserReport_".date('d-m-Y H:i:s').".csv";
            }
    ...
    protected function exportToCSV($searchParam, $input, $output)
    {
        ...
        for ($i = 0; $i <= $count;) {
            ...
            $command = $this->getContainer()->getParameter('fa.php.path').$memoryLimit.' '.$this->getContainer()->getParameter('project_path').'/console fa:export-user-report '.$commandOptions;
        ...
        if (isset($searchParam['rus_csv_email']) && $searchParam['rus_csv_email']) {
            ...
            ->setBody(
                $this->getContainer()->get('templating')->render(
                    'FaReportBundle:UserReportAdmin:email.html.twig',
    ...
    protected function exportToCSVWithOffset($searchParam, $input, $output)
    {
        ...
        $query     = CommonManager::getHistoryRepository($container, 'FaReportBundle:UserReport')->getUserReportQuery($criteria, $criteria['sorter']);
        $offset    = $input->getOption('offset');
        ...
        if (count($results) > 0) {
            ...
            if (CommonManager::inArrayMulti($categoryFields, $criteria['rus_report_columns']) || CommonManager::inArrayMulti($editionFields, $criteria['rus_report_columns']) || CommonManager::inArrayMulti($packageFields, $criteria['rus_report_columns'])) {
                ...
                if (CommonManager::inArrayMulti($editionFields, $criteria['rus_report_columns'])) {
                    if ($userIdsArray && is_array($userIdsArray)) {
                        $editionArray = CommonManager::getHistoryRepository($container, 'FaReportBundle:UserReportEditionDaily')->getEditionInWhichMaxAdPostedByUserIdAndDateRange($userIdsArray, $criteria['rus_from_date'], $criteria['rus_to_date']);
            ...
            $file = fopen($container->get('kernel')->getRootDir()."/../data/reports/user/".$fileName, "a+");
            foreach ($results as $record) {
                $processedRecord = CommonManager::getHistoryRepository($container, 'FaReportBundle:UserReport')->processRecord($record, $criteria, $container);
                $recordValues   = array();
    ...
    {
        $container = $this->getContainer();
        $query     = CommonManager::getHistoryRepository($container, 'FaReportBundle:UserReport')->getUserReportQuery($searchParam);
