...
EOF
    ...
    protected function updateAdEnquiryDaily($input, $output)
    {
        ...
        for ($i = 0; $i < $count;) {
            ...
            $command = $this->getContainer()->getParameter('fa.php.path').$memoryLimit.' '.$this->getContainer()->get('kernel')->getRootDir().'/console fa:update:ad-enquiry-daily-report '.$commandOptions.' '.$input->getArgument('action');
