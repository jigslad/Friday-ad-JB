...
EOF
    ...
    protected function updateAdEnquiryDaily($input, $output)
    {
        ...
        for ($i = 0; $i < $count;) {
            ...
            $command = $this->getContainer()->getParameter('fa.php.path').$memoryLimit.' '.$this->getContainer()->getParameter('project_path').'/console fa:update:ad-enquiry-daily-report '.$commandOptions.' '.$input->getArgument('action');
    ...
    protected function updateAdEnquiryDailyWithOffset($input, $output)
    {
        ...
        if ($adViewCounterArray && count($adViewCounterArray)) {
            ...
            foreach ($adViewedIdsArray as $adId) {
                ...
                if (!$contact_seller_click) {
                    $contact_seller_click = 0;
                }
