 ...
 * @version 1.0
 */
class ExportProfilePackageRevenueReportCommand extends ContainerAwareCommand
{
    /**
    ...
    {
        $this
        ->setName('fa:export-profile-package-revenue-report')
...
Command:
 - php app/console fa:export-profile-package-revenue-report --criteria="xxxx"
EOF
        );
    }
    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
        ...
        if (isset($criteria['rus_csv_name']) && $criteria['rus_csv_name'] && !isset($offset)) {
            ...
            $fileName = "ProfilePackageRevenueReport_".date('d-m-Y H:i:s').".tmp";
        }

        if (isset($offset)) {
            $this->exportToCSVWithOffset($criteria, $input, $output);
        } else {
            $profilePackageRevenueReportRepository = CommonManager::getHistoryRepository($container, 'FaReportBundle:UserReportProfilePackageDaily');
    ...
    protected function exportToCSV($searchParam, $input, $output)
    {
        ...
        for ($i = 0; $i <= $count;) {
            ...
            $command = $this->getContainer()->getParameter('fa.php.path').$memoryLimit.' '.$this->getContainer()->getParameter('project_path').'/console fa:export-profile-package-revenue-report '.$commandOptions;
        ...
        if (isset($searchParam['rus_csv_email']) && $searchParam['rus_csv_email']) {
            ...
            ->setBody(
                $this->getContainer()->get('templating')->render(
                    'FaReportBundle:ProfilePackageRevenueReportAdmin:email.html.twig',
    ...
    protected function exportToCSVWithOffset($searchParam, $input, $output)
    {
        ...
        $query     = CommonManager::getHistoryRepository($container, 'FaReportBundle:UserReport')->getPPRReportQuery($criteria, $criteria['sorter']);
        $offset    = $input->getOption('offset');
        ...
        if (count($results) > 0) {
            ...
            $categoryFields = array_keys(CommonManager::getHistoryRepository($container, 'FaReportBundle:UserReportProfilePackageDaily')->getPPRReportCategoryFieldsArray());
            $isCategorySet  = false;
            if (CommonManager::inArrayMulti($categoryFields, $criteria['rus_report_columns']) && $isCategorySet == false) {
                $categoryArray = CommonManager::getHistoryRepository($container, 'FaReportBundle:UserReportCategoryDaily')->getCategoryInWhichMaxAdPostedByUserIdAndDateRange($userIdsArray, $criteria['rus_from_date'], $criteria['rus_to_date']);
            ...
            $adCountsFields = array_keys(CommonManager::getHistoryRepository($container, 'FaReportBundle:UserReportProfilePackageDaily')->getPPRReportDailyBasicFieldsArray());
            if (CommonManager::inArrayMulti($adCountsFields, $criteria['rus_report_columns'])) {
                $adCountsArray = CommonManager::getHistoryRepository($container, 'FaReportBundle:UserReportDaily')->getDifferentSumByUserIdAndDateRange($userIdsArray, $criteria['rus_from_date'], $criteria['rus_to_date']);
            ...
            $packageFields = array_keys(CommonManager::getHistoryRepository($container, 'FaReportBundle:UserReportProfilePackageDaily')->getPPRReportProfilePackageFieldsArray());
            if (CommonManager::inArrayMulti($packageFields, $criteria['rus_report_columns'])) {
                ...
                if ($userIdsArray && is_array($userIdsArray)) {
                    $packageArray = CommonManager::getHistoryRepository($container, 'FaReportBundle:UserReportProfilePackageDaily')->getPackageDetailsByUserIdAndDateRange($userIdsArray, $criteria['rus_from_date'], $criteria['rus_to_date']);
                    if ($packageArray && is_array($packageArray)) {
                        foreach ($resultArray as $keyUserId => $valueFields) {
                            if (array_key_exists($keyUserId, $packageArray)) {
                                $packageName = CommonManager::getHistoryRepository($container, 'FaReportBundle:UserReportProfilePackageDaily')->getMostRecentPackageNameByUserIdAndDateRange($keyUserId, $criteria['rus_from_date'], $criteria['rus_to_date']);
                    ...
                    $packageRevenueArray = CommonManager::getHistoryRepository($container, 'FaReportBundle:UserReportProfilePackageDaily')->getPackageRevenueDetailsByUserIdAndDateRange($userIdsArray, $criteria['rus_from_date'], $criteria['rus_to_date']);
                    ...
                    $packageCancelledArray = CommonManager::getHistoryRepository($container, 'FaReportBundle:UserReportProfilePackageDaily')->getPackageCancelledCountsByUserIdAndDateRange($userIdsArray, $criteria['rus_from_date'], $criteria['rus_to_date']);
            ...
            $file = fopen($container->get('kernel')->getRootDir()."/../data/reports/profile_package_revenue/".$fileName, "a+");
            foreach ($results as $record) {
                $processedRecord = CommonManager::getHistoryRepository($container, 'FaReportBundle:UserReport')->processRecordForPPR($record, $criteria, $container);
                $recordValues   = array();
    ...
    {
        $container = $this->getContainer();
        $query     = CommonManager::getHistoryRepository($container, 'FaReportBundle:UserReport')->getPPRReportQuery($searchParam);
