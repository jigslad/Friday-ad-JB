 ...
 * @version 1.0
 */
class ExportUserProfileReportCommand extends ContainerAwareCommand
{
    /**
    ...
    {
        $this
        ->setName('fa:export-user-profile-report')
...
Command:
 - php app/console fa:export-user-profile-report --criteria="xxxx"
EOF
        );
    }
    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
        ...
        if (isset($criteria['rus_csv_name']) && $criteria['rus_csv_name'] && !isset($offset)) {
            ...
            $fileName = "UserProfileReport_".date('d-m-Y H:i:s').".tmp";
        }
    ...
    protected function exportToCSV($searchParam, $input, $output)
    {
        ...
        for ($i = 0; $i <= $count;) {
            ...
            $command = $this->getContainer()->getParameter('fa.php.path').$memoryLimit.' '.$this->getContainer()->getParameter('project_path').'/console fa:export-user-profile-report '.$commandOptions;
        ...
        if (isset($searchParam['rus_csv_email']) && $searchParam['rus_csv_email']) {
            ...
            ->setBody(
                $this->getContainer()->get('templating')->render(
                    'FaReportBundle:UserProfileReportAdmin:email.html.twig',
    ...
    protected function exportToCSVWithOffset($searchParam, $input, $output)
    {
        ...
        $query     = CommonManager::getHistoryRepository($container, 'FaReportBundle:UserReport')->getUserProfileReportQuery($criteria, $criteria['sorter']);
        $offset    = $input->getOption('offset');
        ...
        if (count($results) > 0) {
            ...
            $file = fopen($container->get('kernel')->getRootDir()."/../data/reports/user_profile/".$fileName, "a+");
            foreach ($results as $record) {
                $processedRecord = CommonManager::getHistoryRepository($container, 'FaReportBundle:UserReport')->processUserProfileRecord($record, $criteria, $container);
                $recordValues   = array();
    ...
    {
        $container = $this->getContainer();
        $query     = CommonManager::getHistoryRepository($container, 'FaReportBundle:UserReport')->getUserProfileReportQuery($searchParam);
