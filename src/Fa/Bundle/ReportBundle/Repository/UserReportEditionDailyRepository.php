 ...
 * @version 1.0
 */
class UserReportEditionDailyRepository extends EntityRepository
{
     ...
     */
    public function removeUserReportEditionDailyByDate($date)
    {
        if ($date) {
    ...
    /**
     * Get edition in which maximum ad posted
     *
     * @param integer $userId    user id.
     * @param string  $startDate start date.
     * @param string  $endDate   end date.
     */
    public function getEditionInWhichMaxAdPostedByUserIdAndDateRange($userId, $startDate, $endDate)
    {
        ...
        ->select(self::ALIAS.'.user_id', self::ALIAS.'.edition_id', 'COUNT('.self::ALIAS.'.edition_id) As EditionCount')
        ->where(self::ALIAS.'.user_id IN (:userId)')
        ->setParameter('userId', $userId)
        ->andWhere('('.self::ALIAS.'.created_at BETWEEN '.$finalStartDate.' AND  '.$finalEndDate.')')
        ->groupBy(self::ALIAS.'.user_id', self::ALIAS.'.edition_id')
        ->orderBy('EditionCount', 'DESC');
        //->setFirstResult(0)
        //->setMaxResults(1);
     ...
     */
    public function getEditionInWhichMaxAdPostedByDateRange($searchParams)
    {
        $finalStartDate = '';
        $finalEndDate   = '';
        ...
        $qb = $this->createQueryBuilder(self::ALIAS)
        ->select(self::ALIAS.'.edition_id', 'COUNT('.self::ALIAS.'.edition_id) As EditionCount')
