...
class AdEnquiryReportDailyRepository extends EntityRepository
{
     ...
     * @return array
     */
    public function getAdEnquiryTotalsByAdId($adId = array(), $time = false)
    {
        $qb = $this->getBaseQueryBuilder()
        ->select('SUM('.self::ALIAS.'.call_click) As total_call_click', 'SUM('.self::ALIAS.'.email_send_link) As total_email_send_link', self::ALIAS.'.ad_id')
        ->groupBy(self::ALIAS.'.ad_id');
        ...
        $adEnquiryTotalsArray = array();
        $adEnquiryTotals      = $qb->getQuery()->getArrayResult();
        if (count($adEnquiryTotals)) {
            foreach ($adEnquiryTotals as $adEnquiryTotal) {
                $adEnquiryTotalsArray[$adEnquiryTotal['ad_id']] = array("total_call_click" => $adEnquiryTotal['total_call_click'], "total_email_send_link" => $adEnquiryTotal['total_email_send_link']);
            }
        }

        return $adEnquiryTotalsArray;
    }
}
