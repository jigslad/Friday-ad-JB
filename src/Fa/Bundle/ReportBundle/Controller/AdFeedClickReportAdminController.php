...
use Fa\Bundle\ReportBundle\Form\AdFeedClickSearchAdminType;
...
class AdFeedClickReportAdminController extends CoreController implements ResourceAuthorizationController
{
    ...
    public function indexAction(Request $request)
    {
        ...
        $this->get('fa.searchfilters.manager')->init($this->getHistoryRepository('FaReportBundle:AdFeedClickReportDaily'), $this->getHistoryRepositoryTable('FaReportBundle:AdFeedClickReportDaily'), 'fa_report_ad_feed_click_search_admin');
        $data = $this->get('fa.searchfilters.manager')->getFiltersData();
        // initialize form manager service
        $formManager    = $this->get('fa.formmanager');
        $form           = $formManager->createForm(AdFeedClickSearchAdminType::class, null, array('action' => $this->generateUrl('fa_ad_feed_click_report'), 'method' => 'GET'));
        $pagination     = null;
        ...
        if ($data['search']) {
            ...
            if ($form->isValid()) {
                $query = $this->getHistoryRepository('FaReportBundle:AdFeedClickReportDaily')->getAdFeedClickReportDailyQuery($data['search'], $data['sorter'], $this->container);

                if ($data['search']['feed_report_type'] == 'all') {
                    $query->setHint(Query::HINT_CUSTOM_OUTPUT_WALKER, 'Fa\Bundle\ReportBundle\Walker\AdFeedClickReportSqlWalker');
        ...
        return $this->render('FaReportBundle:AdFeedClickReportAdmin:index.html.twig', $parameters);
    }
    ...
    public function exportAdFeedClickReportToCsvAction(Request $request)
    {
        if (!$this->get('session')->has('ad_feed_click_report_export_criteria')) {
            ...
            exec('nohup'.' '.$this->container->getParameter('fa.php.path').' '.$this->container->getParameter('project_path').'/console fa:export-ad-feed-click-report --criteria=\''.$this->get('session')->get('ad_feed_click_report_export_criteria').'\' >/dev/null &');
    ...
    public function ajaxListAdFeedClickReportCsvAction(Request $request)
    {
        ...
        if ($request->isXmlHttpRequest()) {
            ...
            $htmlContent = $this->renderView('FaReportBundle:AdFeedClickReportAdmin:ajaxListAdFeedClickReportCsv.html.twig', array('fileList' => $fileList));
    ...
    public function ajaxDeleteAdFeedClickReportCsvAction(Request $request)
    {
        ...
        if ($request->isXmlHttpRequest()) {
            ...
            if (is_file($reportPath.$fileName)) {
                ...
                $htmlContent = $this->renderView('FaReportBundle:AdFeedClickReportAdmin:ajaxListAdFeedClickReportCsv.html.twig', array('fileList' => $fileList, 'csvDelete' => 1));
            }
