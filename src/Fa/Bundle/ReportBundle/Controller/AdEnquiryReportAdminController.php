...
use Fa\Bundle\ReportBundle\Form\AdEnquiryReportSearchAdminType;
...
class AdEnquiryReportAdminController extends CoreController implements ResourceAuthorizationController
{
    ...
    public function indexAction(Request $request)
    {
        ...
        $this->get('fa.searchfilters.manager')->init($this->getHistoryRepository('FaReportBundle:AdEnquiryReport'), $this->getHistoryRepositoryTable('FaReportBundle:AdEnquiryReportDaily'), 'fa_report_ad_enquiry_search_admin');
        $data = $this->get('fa.searchfilters.manager')->getFiltersData();
        // initialize form manager service
        $formManager    = $this->get('fa.formmanager');
        $form           = $formManager->createForm(AdEnquiryReportSearchAdminType::class, null, array('action' => $this->generateUrl('fa_report_ad_enquiry'), 'method' => 'GET'));
        $pagination     = null;
        ...
        if ($data['search']) {
            ...
            if ($form->isValid()) {
                $query = $this->getHistoryRepository('FaReportBundle:AdEnquiryReport')->getAdEnquiryReportQuery($data['search'], $data['sorter'], $this->container);
        ...
        return $this->render('FaReportBundle:AdEnquiryReportAdmin:index.html.twig', $parameters);
    }
    ...
    public function exportAdEnquiryReportToCsvAction(Request $request)
    {
        if (!$this->get('session')->has('ad_enquiry_report_export_criteria')) {
            return parent::handleMessage('Please select report criteria.', 'fa_report_ad_enquiry', array(), 'error');
        } else {
            CommonManager::setAdminBackUrl($request, $this->container);
            $backUrl = CommonManager::getAdminCancelUrl($this->container);
            exec('nohup'.' '.$this->container->getParameter('fa.php.path').' '.$this->container->getParameter('project_path').'/console fa:export-ad-enquiry-report --criteria=\''.$this->get('session')->get('ad_enquiry_report_export_criteria').'\' >/dev/null &');
            ...
            return parent::handleMessage($message, ($backUrl ? $backUrl : 'fa_report_ad_enquiry'));
        }
    }
    ...
    public function ajaxListAdEnquiryReportCsvAction(Request $request)
    {
        ...
        if ($request->isXmlHttpRequest()) {
            ...
            $htmlContent = $this->renderView('FaReportBundle:AdEnquiryReportAdmin:ajaxListAdEnquiryReportCsv.html.twig', array('fileList' => $fileList));
    ...
    public function ajaxDeleteAdEnquiryReportCsvAction(Request $request)
    {
        ...
        if ($request->isXmlHttpRequest()) {
            ...
            if (is_file($reportPath.$fileName)) {
                ...
                $htmlContent = $this->renderView('FaReportBundle:AdEnquiryReportAdmin:ajaxListAdEnquiryReportCsv.html.twig', array('fileList' => $fileList, 'csvDelete' => 1));
            }
