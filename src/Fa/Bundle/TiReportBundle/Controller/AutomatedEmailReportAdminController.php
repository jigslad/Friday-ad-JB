...
use Fa\Bundle\TiReportBundle\Form\AutomatedEmailReportSearchAdminType;
...
class AutomatedEmailReportAdminController extends CoreController implements ResourceAuthorizationController
{
    ...
    public function indexAction(Request $request)
    {
        ...
        $this->get('fa.searchfilters.manager')->init($this->getTiHistoryRepository('FaTiReportBundle:AutomatedEmailReportDaily'), $this->getTiHistoryRepositoryTable('FaTiReportBundle:AutomatedEmailReportDaily'), 'fa_ti_report_automated_email_search_admin');
        $data = $this->get('fa.searchfilters.manager')->getFiltersData();

        // initialize form manager service
        $formManager    = $this->get('fa.formmanager');
        $form           = $formManager->createForm(AutomatedEmailReportSearchAdminType::class, null, array('action' => $this->generateUrl('fa_ti_report_automated_email'), 'method' => 'GET', 'allow_extra_fields' =>true));
        $pagination     = null;
        ...
        if ($data['search']) {
            ...
            if ($form->isValid()) {
                ...
                if (isset($data['search']) && !isset($data['search']['parsed']) && $request->getQueryString()) {
                    ...
                    $parsedArray['fa_ti_report_automated_email_search_admin']['parsed'] = true;
                    $parsedArray = array_filter(array_map('array_filter', $parsedArray));

                    return $this->redirectToRoute('fa_ti_report_automated_email', $parsedArray);
                }
                $query = $this->getTiHistoryRepository('FaTiReportBundle:AutomatedEmailReportDaily')->getAutomatedEmailReportQuery($data['search'], $data['sorter'], $this->container);
        ...
        $parameters = array(
            'heading'         => 'TI Automated email report',
        ...
        return $this->render('FaTiReportBundle:AutomatedEmailReportAdmin:index.html.twig', $parameters);
    }
    ...
    public function exportAutomatedEmailReportToCsvAction(Request $request)
    {
        if (!$this->get('session')->has('automated_email_report_export_criteria')) {
            return parent::handleMessage('Please select report criteria.', 'fa_ti_report_automated_email', array(), 'error');
        } else {
            CommonManager::setAdminBackUrl($request, $this->container);
            $backUrl = CommonManager::getAdminCancelUrl($this->container);
            exec('nohup'.' '.$this->container->getParameter('fa.php.path').' '.$this->container->getParameter('project_path').'/console fa:ti:automated-email:report:export-to-csv --criteria=\''.$this->get('session')->get('automated_email_report_export_criteria').'\' >/dev/null &');
            ...
            return parent::handleMessage($message, ($backUrl ? $backUrl : 'fa_ti_report_automated_email'));
        }
    }
    ...
    public function ajaxListAutomatedEmailReportCsvAction(Request $request)
    {
        ...
        if ($request->isXmlHttpRequest()) {
            ...
            $htmlContent = $this->renderView('FaTiReportBundle:AutomatedEmailReportAdmin:ajaxListAutomatedEmailReportCsv.html.twig', array('fileList' => $fileList));
    ...
    public function ajaxDeleteAutomatedEmailReportCsvAction(Request $request)
    {
        ...
        if ($request->isXmlHttpRequest()) {
            ...
            if (is_file($reportPath.$fileName)) {
                ...
                $htmlContent = $this->renderView('FaTiReportBundle:AutomatedEmailReportAdmin:ajaxListAutomatedEmailReportCsv.html.twig', array('fileList' => $fileList, 'csvDelete' => 1));
            }
