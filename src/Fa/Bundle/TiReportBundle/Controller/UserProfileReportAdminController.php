...
use Fa\Bundle\TiReportBundle\Form\UserProfileReportSearchAdminType;
...
class UserProfileReportAdminController extends CoreController implements ResourceAuthorizationController
{
    ...
    public function indexAction(Request $request)
    {
        ...
        $this->get('fa.searchfilters.manager')->init($this->getTiHistoryRepository('FaTiReportBundle:UserReport'), $this->getTiHistoryRepositoryTable('FaTiReportBundle:UserReport'), 'fa_ti_report_user_profile_report_search_admin');
        $data = $this->get('fa.searchfilters.manager')->getFiltersData();
        // initialize form manager service
        $formManager    = $this->get('fa.formmanager');
        $form           = $formManager->createForm(UserProfileReportSearchAdminType::class, null, array('action' => $this->generateUrl('fa_ti_report_user_profile'), 'method' => 'GET'));
        $resultArray    = array();
        $pagination     = null;
        $isExport       = false;
        ...
        if ($data['search']) {
            ...
            if ($form->isValid()) {
                $query = $this->getTiHistoryRepository('FaTiReportBundle:UserReport')->getUserProfileReportQuery($data['search'], $data['sorter']);
                ...
                if ($pagination->getNbResults()) {
                    foreach ($pagination->getCurrentPageResults() as $record) {
                        $processedRecord                          = $this->getTiHistoryRepository('FaTiReportBundle:UserReport')->processUserProfileRecord($record, $data['search'], $this->container);
        ...
        $parameters = array(
            'heading'         => 'TI Users Profile Report',
        ...
        return $this->render('FaTiReportBundle:UserProfileReportAdmin:index.html.twig', $parameters);
    }
    ...
    public function exportToCSVAction(Request $request)
    {
        if (!$this->get('session')->has('search_criteria')) {
            return parent::handleMessage('Please select report criteria.', 'fa_ti_report_user_profile', array(), 'error');
        } else {
            ...
            exec('nohup'.' '.$this->container->getParameter('fa.php.path').' '.$this->container->getParameter('project_path').'/console fa:ti:export-user-profile-report --criteria=\''.$serilizedSearchCriteria.'\' >/dev/null &');
            ...
            return parent::handleMessage($message, ($backUrl ? $backUrl : 'fa_ti_report_user_profile'));
        }
    }
    ...
    public function ajaxListUserProfileReportCsvAction(Request $request)
    {
        ...
        if ($request->isXmlHttpRequest()) {
            ...
            $htmlContent = $this->renderView('FaTiReportBundle:UserProfileReportAdmin:ajaxListUserProfileReportCsv.html.twig', array('fileList' => $fileList));
    ...
    public function ajaxDeleteUserProfileReportCsvAction(Request $request)
    {
        ...
        if ($request->isXmlHttpRequest()) {
            ...
            if (is_file($reportPath.$fileName)) {
                ...
                $htmlContent = $this->renderView('FaTiReportBundle:UserProfileReportAdmin:ajaxListUserProfileReportCsv.html.twig', array('fileList' => $fileList, 'csvDelete' => 1));
            }
