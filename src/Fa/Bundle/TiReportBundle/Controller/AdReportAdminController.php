...
class AdReportAdminController extends CoreController implements ResourceAuthorizationController
{
    ...
    public function indexAction(Request $request)
    {
        ...
        $form           = $formManager->createForm(AdReportSearchAdminType::class, null, array('action' => $this->generateUrl('fa_ti_report_ad'), 'method' => 'GET'));
        $pagination     = null;
    ...
    public function exportAdReportToCsvAction(Request $request)
    {
        if (!$this->get('session')->has('ad_report_export_criteria')) {
            ...
            exec('nohup'.' '.$this->container->getParameter('fa.php.path').' '.$this->container->get('kernel')->getRootDir().'/console fa:ti:ad:report:export-to-csv --criteria=\''.$this->get('session')->get('ad_report_export_criteria').'\' >/dev/null &');
