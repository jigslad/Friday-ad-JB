...
use Fa\Bundle\TiReportBundle\Form\ProfilePackageRevenueReportSearchAdminType;
...
class ProfilePackageRevenueReportAdminController extends CoreController implements ResourceAuthorizationController
{
    ...
    public function indexAction(Request $request)
    {
        ...
        $this->get('fa.searchfilters.manager')->init($this->getTiHistoryRepository('FaTiReportBundle:UserReport'), $this->getTiHistoryRepositoryTable('FaTiReportBundle:UserReport'), 'fa_ti_report_profile_package_revenue_report_search_admin');
        $data = $this->get('fa.searchfilters.manager')->getFiltersData();
        // initialize form manager service
        $formManager    = $this->get('fa.formmanager');
        $form           = $formManager->createForm(ProfilePackageRevenueReportSearchAdminType::class, null, array('action' => $this->generateUrl('fa_ti_report_profile_package_revenue'), 'method' => 'GET'));
        $resultArray    = array();
        $pagination     = null;
        $isExport       = false;
        ...
        if ($data['search']) {
            ...
            if ($form->isValid()) {
                $query = $this->getTiHistoryRepository('FaTiReportBundle:UserReport')->getPPRReportQuery($data['search'], $data['sorter']);
                ...
                if ($pagination->getNbResults()) {
                    foreach ($pagination->getCurrentPageResults() as $record) {
                        $processedRecord                          = $this->getTiHistoryRepository('FaTiReportBundle:UserReport')->processRecordForPPR($record, $data['search'], $this->container);
                        $userIdsArray[]                           = $processedRecord['user_id'];
                        $resultArray[$processedRecord['user_id']] = $processedRecord;
                    }


                    $categoryFields = array_keys(CommonManager::getTiHistoryRepository($this->container, 'FaTiReportBundle:UserReportProfilePackageDaily')->getPPRReportCategoryFieldsArray());
                    $isCategorySet  = false;
                    if (CommonManager::inArrayMulti($categoryFields, $data['search']['rus_report_columns']) && $isCategorySet == false) {
                        $categoryArray = CommonManager::getTiHistoryRepository($this->container, 'FaTiReportBundle:UserReportCategoryDaily')->getCategoryInWhichMaxAdPostedByUserIdAndDateRange($userIdsArray, $data['search']['rus_from_date'], $data['search']['rus_to_date']);
                    ...
                    $adCountsFields = array_keys(CommonManager::getTiHistoryRepository($this->container, 'FaTiReportBundle:UserReportProfilePackageDaily')->getPPRReportDailyBasicFieldsArray());
                    if (CommonManager::inArrayMulti($adCountsFields, $data['search']['rus_report_columns'])) {
                        $adCountsArray = CommonManager::getTiHistoryRepository($this->container, 'FaTiReportBundle:UserReportDaily')->getDifferentSumByUserIdAndDateRange($userIdsArray, $data['search']['rus_from_date'], $data['search']['rus_to_date']);
                    ...
                    $packageFields = array_keys(CommonManager::getTiHistoryRepository($this->container, 'FaTiReportBundle:UserReportProfilePackageDaily')->getPPRReportProfilePackageFieldsArray());
                    if (CommonManager::inArrayMulti($packageFields, $data['search']['rus_report_columns'])) {
                        if ($userIdsArray && is_array($userIdsArray)) {
                            $packageArray = CommonManager::getTiHistoryRepository($this->container, 'FaTiReportBundle:UserReportProfilePackageDaily')->getPackageDetailsByUserIdAndDateRange($userIdsArray, $data['search']['rus_from_date'], $data['search']['rus_to_date']);
                            if ($packageArray && is_array($packageArray)) {
                                foreach ($resultArray as $keyUserId => $valueFields) {
                                    if (array_key_exists($keyUserId, $packageArray)) {
                                        $packageName = CommonManager::getTiHistoryRepository($this->container, 'FaTiReportBundle:UserReportProfilePackageDaily')->getMostRecentPackageNameByUserIdAndDateRange($keyUserId, $data['search']['rus_from_date'], $data['search']['rus_to_date']);
                            ...
                            $packageRevenueArray = CommonManager::getTiHistoryRepository($this->container, 'FaTiReportBundle:UserReportProfilePackageDaily')->getPackageRevenueDetailsByUserIdAndDateRange($userIdsArray, $data['search']['rus_from_date'], $data['search']['rus_to_date']);
                            ...
                            $packageCancelledArray = CommonManager::getTiHistoryRepository($this->container, 'FaTiReportBundle:UserReportProfilePackageDaily')->getPackageCancelledCountsByUserIdAndDateRange($userIdsArray, $data['search']['rus_from_date'], $data['search']['rus_to_date']);
        ...
        $parameters = array(
            'heading'         => 'TI Profile Package Revenue Report',
        ...
        return $this->render('FaTiReportBundle:ProfilePackageRevenueReportAdmin:index.html.twig', $parameters);
    }
    ...
    public function exportToCSVAction(Request $request)
    {
        if (!$this->get('session')->has('search_criteria')) {
            return parent::handleMessage('Please select report criteria.', 'fa_ti_report_profile_package_revenue', array(), 'error');
        } else {
            ...
            exec('nohup'.' '.$this->container->getParameter('fa.php.path').' '.$this->container->getParameter('project_path').'/console fa:ti:export-profile-package-revenue-report --criteria=\''.$serilizedSearchCriteria.'\' >/dev/null &');
            ...
            return parent::handleMessage($message, ($backUrl ? $backUrl : 'fa_ti_report_user'));
        }
    }
    ...
    public function ajaxListPPRReportCsvAction(Request $request)
    {
        ...
        if ($request->isXmlHttpRequest()) {
            ...
            $htmlContent = $this->renderView('FaTiReportBundle:ProfilePackageRevenueReportAdmin:ajaxListPPRReportCsv.html.twig', array('fileList' => $fileList));
    ...
    public function ajaxDeletePPRReportCsvAction(Request $request)
    {
        ...
        if ($request->isXmlHttpRequest()) {
            ...
            if (is_file($reportPath.$fileName)) {
                ...
                $htmlContent = $this->renderView('FaTiReportBundle:ProfilePackageRevenueReportAdmin:ajaxListPPRReportCsv.html.twig', array('fileList' => $fileList, 'csvDelete' => 1));
            }
