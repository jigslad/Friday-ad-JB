...
use Fa\Bundle\TiReportBundle\Form\UserReportSearchAdminType;
...
class UserReportAdminController extends CoreController implements ResourceAuthorizationController
{
    ...
    public function indexAction(Request $request)
    {
        ...
        $this->get('fa.searchfilters.manager')->init($this->getTiHistoryRepository('FaTiReportBundle:UserReport'), $this->getTiHistoryRepositoryTable('FaTiReportBundle:UserReport'), 'fa_ti_report_user_report_search_admin');
        $data = $this->get('fa.searchfilters.manager')->getFiltersData();
        // initialize form manager service
        $formManager    = $this->get('fa.formmanager');
        $form           = $formManager->createForm(UserReportSearchAdminType::class, null, array('action' => $this->generateUrl('fa_ti_report_user'), 'method' => 'GET'));
        $resultArray    = array();
        $pagination     = null;
        $isExport       = false;
        ...
        if ($data['search']) {
            ...
            if ($form->isValid()) {
                if ($data['search']['rus_report_type'] == 'user_wise') {
                    $query = $this->getTiHistoryRepository('FaTiReportBundle:UserReport')->getUserReportQuery($data['search'], $data['sorter']);
                    ...
                    if ($pagination->getNbResults()) {
                        foreach ($pagination->getCurrentPageResults() as $record) {
                            $processedRecord                          = $this->getTiHistoryRepository('FaTiReportBundle:UserReport')->processRecord($record, $data['search'], $this->container);
                        ...
                        if (CommonManager::inArrayMulti($editionFields, $data['search']['rus_report_columns'])) {
                            if ($userIdsArray && is_array($userIdsArray)) {
                                $editionArray = CommonManager::getTiHistoryRepository($this->container, 'FaTiReportBundle:UserReportEditionDaily')->getEditionInWhichMaxAdPostedByUserIdAndDateRange($userIdsArray, $data['search']['rus_from_date'], $data['search']['rus_to_date']);
                    ...
                    if (CommonManager::inArrayMulti($reportDailyFields, $data['search']['rus_report_columns'])) {
                        $resultArray = $this->getTiHistoryRepository('FaTiReportBundle:UserReportDaily')->getUserReportDailyTotalSum($data['search']);
                    } else {
                        $resultArray[0] = array();
                    }
                    ...
                    $categoryAndEditionDataArray = $this->getTiHistoryRepository('FaTiReportBundle:UserReport')->getCategoryAndEditionDataArray($data['search'], $this->container);

                    if (CommonManager::inArrayMulti($reportBooleanFields, $data['search']['rus_report_columns'])) {
                        $booleanAndOtherFieldsArray = $this->getTiHistoryRepository('FaTiReportBundle:UserReport')->getBooleanAndOtherFieldsSumQuery($data['search'])->getResult();
                    }

                    if (CommonManager::inArrayMulti(array('is_new', 'signup_date'), $data['search']['rus_report_columns'])) {
                        $newlySignupArray = $this->getTiHistoryRepository('FaTiReportBundle:UserReport')->getNewlySignupUsersQuery($data['search'])->getResult();
                    }
        ...
        return $this->render('FaTiReportBundle:UserReportAdmin:index.html.twig', $parameters);
    }
    ...
    public function exportToCSVAction(Request $request)
    {
        if (!$this->get('session')->has('search_criteria')) {
            return parent::handleMessage('Please select report criteria.', 'fa_ti_report_user', array(), 'error');
        } else {
            ...
            exec('nohup'.' '.$this->container->getParameter('fa.php.path').' '.$this->container->getParameter('project_path').'/console fa:ti:export-user-report --criteria=\''.$serilizedSearchCriteria.'\' >/dev/null &');
    ...
    public function ajaxListUserReportCsvAction(Request $request)
    {
        ...
        if ($request->isXmlHttpRequest()) {
            ...
            $htmlContent = $this->renderView('FaTiReportBundle:UserReportAdmin:ajaxListUserReportCsv.html.twig', array('fileList' => $fileList));
    ...
    public function ajaxDeleteUserReportCsvAction(Request $request)
    {
        ...
        if ($request->isXmlHttpRequest()) {
            ...
            if (is_file($reportPath.$fileName)) {
                ...
                $htmlContent = $this->renderView('FaTiReportBundle:UserReportAdmin:ajaxListUserReportCsv.html.twig', array('fileList' => $fileList, 'csvDelete' => 1));
            }
