...
use Fa\Bundle\TiReportBundle\Repository\AdPrintInsertDateReportDailyRepository;
...
class AdReportDailyRepository extends EntityRepository
{
    ...
    public function getAdReportQuery($searchParams, $sorter, $container, $isCountQuery = false)
    {
        ...
        if (in_array('print_insert_date', $searchParams['report_columns']) || $searchParams['date_filter_type'] == 'print_insert_date') {
    ...
    public function getAdPrintInsertDatesByAdIds($adIds, $searchParams, $sorter, $container)
    {
        ...
        foreach ($adPrintDates as $adPrintDate) {
            ...
            if (!isset($adPrintEditionArr[$adPrintDate['ad_id']][$adPrintDate['id']]) || !in_array($adPrintDate['print_edition_id'], $adPrintEditionArr[$adPrintDate['ad_id']][$adPrintDate['id']])) {
                $adPrintEditionArr[$adPrintDate['ad_id']][$adPrintDate['id']][] = $adPrintDate['print_edition_id'];
            }
        }
    ...
    private function addFilter($qb, $searchParams, $sorter, $container, $isPrintPublishedFlag = false, $isCountQuery = false, $isPrintPublishedGroupByAdFlag = false)
    {
        ...
        if ($isPrintPublishedFlag) {
            $qb->leftJoin('FaTiReportBundle:AdPrintInsertDateReportDaily', AdPrintInsertDateReportDailyRepository::ALIAS, 'WITH', AdPrintInsertDateReportDailyRepository::ALIAS.'.ad_report_daily_id = '.self::ALIAS.'.id')
                ->andWhere('('.AdPrintInsertDateReportDailyRepository::ALIAS.'.print_insert_date BETWEEN '.$finalStartDate.' AND  '.$finalEndDate.')')
                ->andWhere(self::ALIAS.'.package_sr_no IN (6,7)')
                ->distinct(self::ALIAS.'.id');
        } else {
            $qb = $qb->andWhere('('.self::ALIAS.'.'.$searchParams['date_filter_type'].' BETWEEN '.$finalStartDate.' AND  '.$finalEndDate.')');
        }
