...
EOF
    ...
    protected function updateEntityCache($searchParam, $action, $entityClass, $input, $output)
    {
        ...
        for ($i = 0; $i <= $count;) {
            ...
            $command = $this->getContainer()->getParameter('fa.php.path').$memoryLimit.' bin/console fa:update:entity '.' '.$input->getArgument('action').' '.$input->getArgument('class').' '.$commandOptions;
