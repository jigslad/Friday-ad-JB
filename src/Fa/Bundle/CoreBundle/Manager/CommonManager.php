...
//             $twig->parse($twig->tokenize($value)); // Twig_Environment::tokenize() must be an instance of Twig_Source
    ...
    {
        $current_year = date('Y');
        $current_year = $current_year-21;
        $years     = array('pre-'.$current_year => 'Pre '.$current_year);
        $yearRange = range($current_year, date("Y"));
    ...
    */
    ...
    public function pc_permute($items, $perms = array()) {                        
        if (empty($items)) { 
             $joinarr = join(' ', $perms);
             $retval[] = $joinarr;
             self::$permuteresult = array_merge(self::$permuteresult,$retval);             
         } else {
