...
use Fa\Bundle\CoreBundle\Manager\GDManager;
    ...
    */
    public function __construct($maxWidth = null, $maxHeight = null, $scale = true, $inflate = true, $quality = 75, $adapterClass = null, $adapterOptions = array())
    {
        if (($adapterClass && $adapterClass == 'GDManager') || (!$adapterClass && extension_loaded('gd'))) {
            $this->adapter = new GDManager($maxWidth, $maxHeight, $scale, $inflate, $quality, $adapterOptions);
        } elseif (($adapterClass && $adapterClass == 'ImageMagickManager') || !$adapterClass) {
            $this->adapter = new ImageMagickManager($maxWidth, $maxHeight, $scale, $inflate, $quality, $adapterOptions);
        }
    }
