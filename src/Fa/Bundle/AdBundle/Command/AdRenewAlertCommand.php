...
EOF
    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
        ...
        if (isset($offset)) {
            ...
            $command = $this->getContainer()->getParameter('fa.php.path').$memoryLimit.' '.$this->getContainer()->getParameter('project_path').'/console fa:process-email-queue --email_identifier="ad_needs_renewing_4_days_left"';
    ...
    protected function updateAdRenewal($searchParam, $input, $output)
    {
        ...
        for ($i = 0; $i <= $count;) {
            ...
            $command = $this->getContainer()->getParameter('fa.php.path').$memoryLimit.' '.$this->getContainer()->getParameter('project_path').'/console fa:update:ad-renew-alert '.$commandOptions;
            $output->writeln($command, true);
            passthru($command, $returnVar);
