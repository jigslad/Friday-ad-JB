...
EOF
    ...
    protected function searchAgentAlert($input, $output)
    {
        ...
        for ($i = 0; $i <= $count;) {
            ...
            $command = $this->getContainer()->getParameter('fa.php.path').$memoryLimit.' '.$this->getContainer()->get('kernel')->getRootDir().'/console fa:update:search-agent-alert '.$commandOptions.' --verbose';
