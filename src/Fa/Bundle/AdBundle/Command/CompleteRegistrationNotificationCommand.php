 ...
 * @version 1.0
 */
class CompleteRegistrationNotificationCommand extends ContainerAwareCommand
{
    /**
     * Configure.
     */
    ...
    {
        $this
        ->setName('fa:send:complete-registration-notification')
        ->setDescription("This notification will sent to people who silently registered with their phone number, 90 minutes after an ad is placed via a PAA Lite form")
...
Command:
 - php app/console fa:send:complete-registration-notification"
EOF
        );
    }
    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
        ...
        if (isset($offset)) {
            $this->completeRegistrationNotificationsWithOffset($input, $output);
        } else {
            $this->completeRegistrationNotifications($input, $output);
    ...
    protected function completeRegistrationNotifications($input, $output)
    {
        ...
        for ($i = 0; $i <= $count;) {
            ...
            $command = $this->getContainer()->getParameter('fa.php.path').$memoryLimit.' '.$this->getContainer()->getParameter('project_path').'/console fa:send:complete-registration-notification '.$commandOptions;
    ...
    protected function getAdQueryBuilder($onlyCount = false, $input)
    {
        ...
        $sql .= ' FROM paa_lite_email_notification as '.PaaLiteEmailNotificationRepository::ALIAS.' WHERE UNIX_TIMESTAMP(date_add(FROM_UNIXTIME('.PaaLiteEmailNotificationRepository::ALIAS.'.created_at), interval +90 minute)) <= UNIX_TIMESTAMP(NOW()) AND '.PaaLiteEmailNotificationRepository::ALIAS.'.is_paa_lite_registered_user = 1 AND '.PaaLiteEmailNotificationRepository::ALIAS.'.is_registration_notification_sent = 0 ORDER BY '.PaaLiteEmailNotificationRepository::ALIAS.'.id ASC';
