 ...
 * @version v1.0
 */
class SyncOldToNewAwsImagesCommand extends ContainerAwareCommand
{
    /**
     * Configure.
     */
    ...
    {
        $this
        ->setName('fa:sync:image-s3-new')
        ->setDescription('Synchronize images to new s3')
    ...
    protected function UploadImagesToAwsS3($input, $output)
    {
        ...
        for ($i = 0; $i <= $count;) {
            ...
            $command = $this->getContainer()->getParameter('fa.php.path').$memoryLimit.' '.$this->getContainer()->getParameter('project_path').'/console fa:sync:image-s3-new '.$commandOptions;
    ...
    protected function getAdQueryBuilder($onlyCount = false, $input)
    {
        ...
        if($last_updated!='') {
            $qb->andWhere(AdImageRepository::ALIAS.'.created_at > :created_at');
            $qb->setParameter('created_at', CommonManager::getTimeStampFromStartDate(date('Y-m-d', strtotime($last_updated))));
        }
