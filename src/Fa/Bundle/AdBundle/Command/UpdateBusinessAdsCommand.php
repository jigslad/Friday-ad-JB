...
use Fa\Bundle\AdBundle\lib\Migration\BusinessAd;
 ...
 * @version 1.0
 */
class UpdateBusinessAdsCommand extends ContainerAwareCommand
{
    /**
     * Configure.
     */
    ...
    {
        $this
        ->setName('fa:update:business-ads')
        ->setDescription("Update business ads")
...
Actions:
Command:
   php app/console update:business-ads
EOF
        );
    }
    ...
    protected function updateDimension($searchParam, $input, $output)
    {
        ...
        for ($i = 0; $i <= $count;) {
            ...
            $command = $this->getContainer()->getParameter('fa.php.path').$memoryLimit.' '.$this->getContainer()->getParameter('project_path').'/console fa:update:business-ads --verbose '.$commandOptions.' '.$input->getArgument('action');
    ...
    protected function getUserQueryBuilder($searchParam)
    {
        ...
        $qb->andWhere(UserRepository::ALIAS.'.update_type IN (:update_type)');
