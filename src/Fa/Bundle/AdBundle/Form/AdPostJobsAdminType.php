...
class AdPostJobsAdminType extends AdPostAdminType
{
    /**
     * Build form.
     *
    ...
    public function postSubmit(FormEvent $event)
    {
        ...
        if ($form->isValid()) {
            if ($form->getData()->getId()) {
                ...
                $data          = $this->request->get('fa_paa_jobs_admin');
    ...
    {
        if ($categoryId) {
            $dimensions = $this->em->getRepository('FaEntityBundle:CategoryDimension')->getDimesionsByCategoryId($categoryId, $this->container);

            foreach ($dimensions as $dimensionId => $dimensionName) {
                $this->addCategroyDimensionChoiceListField($form, $dimensionId, $dimensionName, $adForSale);
            }
        }
    }
    ...
    {
        return CategoryRepository::JOBS_ID;
    }
