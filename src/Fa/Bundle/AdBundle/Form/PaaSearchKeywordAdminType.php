...
/**
 * PaaSearchKeywordAdminType form.
 *
...
class PaaSearchKeywordAdminType extends AbstractType
{
    /**
    ...
    {
        return 'fa_ad_paa_search_keyword_admin';
    }
    ...
    public function preSetData(FormEvent $event)
    {
        ...
        if ($searchKeyword->getId()) {
            $counter     = 1;
            $categoryIds = $this->em->getRepository('FaAdBundle:PaaSearchKeywordCategory')->getCategoryIdsByKeywordId($searchKeyword->getId());
    ...
    public function saveKeywordCategories($form, $searchKeyword)
    {
        ...
        $this->em->getRepository('FaAdBundle:PaaSearchKeywordCategory')->removeByKeywordId($searchKeyword->getId());

        // Insert new entries.
        foreach ($processedKeyword as $processedKeywordInfo) {
            $paaSearchKeywordCategory = new PaaSearchKeywordCategory();
            $paaSearchKeywordCategory->setSearchKeywordId($processedKeywordInfo['search_keyword_id']);
            $paaSearchKeywordCategory->setCategoryId($processedKeywordInfo['category_id']);
            $paaSearchKeywordCategory->setKeyword($processedKeywordInfo['keyword']);
            $paaSearchKeywordCategory->setSearchCount($processedKeywordInfo['search_count']);

            $this->em->persist($paaSearchKeywordCategory);
            $this->em->flush($paaSearchKeywordCategory);
        }
    }
