...
class AdLocationRepository extends EntityRepository
{
     ...
     * @return Apache_Solr_Document
     */
    public function getSolrDocumentNew($ad, $document = null, $container = null, $categoryString = null)
    {
        if (!$document) {
            $document = new \SolrInputDocument($ad);
        }
        ...
        if ($mainTownName) {
            try {
                $document = $this->addField($document, 'ad_detail_url', $container->get('fa_ad.manager.ad_routing')->getAdDetailUrlByDetails($categoryString, $mainTownName, Urlizer::urlize(CommonManager::trimTextByWords($ad->getTitle(), 7, '')), $ad->getId()));
            } catch (\Exception $e) {
                $document = $this->addField($document, 'ad_detail_url', $container->get('fa_ad.manager.ad_routing')->getAdDetailUrlByDetails($categoryString, $mainTownName, Urlizer::urlize($ad->getId()), $ad->getId()));
            }
        }
