        ...
        if ($ad->getUser()) {
            if ($isAssignUserToDetachedAd || $isNewAd) {
                if ($ad->getUser()->getIsPrivatePhoneNumber() && $ad->getUser()->getPhone()) {
                    exec('nohup'.' '.$this->container->getParameter('fa.php.path').' bin/console fa:update:ad-yac-number allocate --ad_id='.$ad->getId().' >/dev/null &');
                } else {
                    exec('nohup'.' '.$this->container->getParameter('fa.php.path').' bin/console fa:update:ad-yac-number setsold --ad_id='.$ad->getId().' >/dev/null &');
                }
                ...
                if ($ad->getUser()->getIsPrivatePhoneNumber() && $ad->getUsePrivacyNumber() && ($oldPhone && $oldPhone != $ad->getPhone())) {
                    exec('nohup'.' '.$this->container->getParameter('fa.php.path').' bin/console fa:update:ad-yac-number edit --ad_id='.$ad->getId().' >/dev/null &');
                }
            }
        } else {
