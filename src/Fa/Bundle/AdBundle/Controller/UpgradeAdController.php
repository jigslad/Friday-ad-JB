...
class UpgradeAdController extends CoreController
{
     ...
     * @return Response|JsonResponse A Response or JsonResponse object.
     */
    public function ajaxUpgradeToNextAdAction($adId, $rootCategoryId, Request $request)
    {
        $redirectToUrl = '';
        $error         = '';
        $htmlContent   = '';
        ...
        if ($request->isXmlHttpRequest()) {
            ...
            if (!empty($loggedinUser)) {
                ...
                if (!empty($user)) {
                    ...
                    if (!empty($activePackage) && $activePackage->getPackage()) {
                        ...
                        //get available fetaured top package
                        $packages = $this->getRepository('FaPromotionBundle:PackageRule')->getPackageByCategoryId($availablePackageIds[0]);
                        
                        $getUserLastAdvert = $this->getRepository('FaAdBundle:Ad')->getUserLastBasicLiveAdvert($user->getId(), $adId, $adRootCategoryId, $this->container);
                        //get Print Edition if exist
                        $printEditionLimits = $this->getRepository('FaPromotionBundle:Package')->getPrintEditionLimitForPackages($availablePackageIds);
