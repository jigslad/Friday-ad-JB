...
class CampaignController extends ThirdPartyLoginController
{
    ...
    {
        $showLoginPopUp = 1;$getCategoryId = 1;$getCategoryLevel = 1;$showAdLivePopUp = 0;
        $showAdLimitPopUp = 0;$getAdPlacedCount = 0;$showErrorPopUp=0;$showAddToCartPopUp = 0;
        if ($this->isAuth()) {
        ...
        $getPaaLiteFlds = array();$getCampaign=array();$rootCategory = array();$vertical = '';
        $getCampaign = $this->getRepository('FaAdBundle:Campaigns')->getCampaignBySlug($campaign_name);
        $CategoryNxtLevel = 0;$getPaaLiteOnlyFlds = array();$rootCategoryId = 0;$getCatLvlArrs = array();
        $getChildrenForCategory = array();$isLastCategory = 0;
        if(!empty($getCampaign)) {
            ...
            if(empty($getChildrenForCategory)) { $isLastCategory = 1; }
            if($rootCategoryId!='') {
            ...
            $getCatPath = array();$getCatLvlArrs = array();
        ...
        if ($request->isXmlHttpRequest()) {
            ...
            if(!empty($loggedinUser)) {
                ...
                if(!empty($user)) {
                    $adId             = $this->container->get('session')->get('cart_ad_id');
                    $ad               = $this->getRepository('FaAdBundle:Ad')->find($adId);
                    $categoryId       = $ad->getCategory()->getId();         $adExpiryDays     = $this->getRepository('FaCoreBundle:ConfigRule')->getExpirationDays($categoryId, $this->container);
                    $printEditionLimits = $this->getRepository('FaPromotionBundle:Package')->getPrintEditionLimitForPackages($packageIds);
