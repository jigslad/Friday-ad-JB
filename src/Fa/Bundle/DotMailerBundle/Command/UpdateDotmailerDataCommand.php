...
EOF
    ...
    protected function updateDimension($input, $output)
    {
        ...
        for ($i = 0; $i <= $count;) {
            ...
            $command = $this->getContainer()->getParameter('fa.php.path').$memoryLimit.' '.$this->getContainer()->get('kernel')->getRootDir().'/console fa:update:dotmailer-data '.$commandOptions.' '.$input->getArgument('action');
